<template>
    <div class="bg-violet-200 flex items-center flex-col justify-center min-h-screen font-rowdies font-bold ">
      <div class="flex max-w-2xl w-full justify-between mb-8">
        ...
        <a class ="float-left w-10" target="_blank" href="https://t.me/zhansuuw"> <img src="https://img.icons8.com/?size=64&id=HTBOiI9JKlWY&format=png" alt=""></a>
      </div>
      <div class="bg-orange-50 border-black border-2 p-2.5 outline-none shadow-[5px_5px_0px_rgba(0,0,0,1)]  max-w-2xl w-full sm:px-10 sm:py-6 flex flex-col justify-center gap-6 items-start">
        <div class="input-div w-full flex gap-2 sm:gap-3">
          ...
          />
          <button @click="searchCity" class="bg-slate-300 border-black border-2 p-2.5 outline-none shadow-[5px_5px_0px_rgba(0,0,0,1)] active:shadow-[2px_2px_0px_rgba(0,0,0,1)] flex items-center justify-center" type="button">
  ...
  export default {
    ...
    methods: {
      ...
      try {
        ...
        const API_URL = `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(this.searchQuery)}&appid=${API_KEY}`;
        ...
        this.weatherData = {
          ...
          description: this.translateDescription(data.weather[0].icon),
